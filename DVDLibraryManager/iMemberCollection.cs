...
namespace DVDLibraryManager
{
    ...
    interface iMemberCollection
    {
        ...
        void delete(int memberIndex); //delete a given member from this member collection, a member can be deleted only when the member currently is not holding any tool

        Boolean search(string contactNumber); //search a given member in this member collection. Return true if this memeber is in the member collection; return false otherwise
